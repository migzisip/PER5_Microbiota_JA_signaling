...
annoSample <- read_csv("D:/Projects/PER5_RNA_seq/Root_DEGs/list_samples.csv")
...
design(degres) <- ~sv1 + Condition  # Only include condition factor
...
cond <- list(
  c("per5_R_M", "Ws4_R_M"),
  c("Ws4_R_SC", "Ws4_R_M"),
  c("per5_R_SC", "Ws4_R_SC"),
  c("per5_R_SC", "per5_R_M")
)
...
res <- cbind.data.frame(as.matrix(mcols(degres)[, 1:10]), counts(degres, normalize = TRUE), stringsAsFactors = FALSE) %>%
  ...
  select(ID, Gene:Description, Ws4_R_M1 : per5_R_SC_vs_per5_R_M_log2FoldChange) %>%  # Select relevant columns
  arrange(per5_R_SC_vs_per5_R_M_pvalue)  

#Export DEG tables
res %>%
  filter(per5_R_SC_vs_per5_R_M_padj <= 0.05,
         per5_R_SC_vs_per5_R_M_log2FoldChange %>% abs > log2(1.5)) %>%
  write_csv("per5_R_M_SC_DEGs.csv")

res %>%
  filter(
    Ws4_R_SC_vs_Ws4_R_M_padj <= 0.05,
    Ws4_R_SC_vs_Ws4_R_M_log2FoldChange %>% abs() > log2(1.5)
  ) %>%
  write_csv("Ws4_R_M_SC_DEGs.csv")

write_csv(res, "root_All_DEGs.csv")
#==================================================================#
...
ggplot(
  ...
  coord_fixed(ratio = 1) +
  scale_colour_manual(values=c("#C99800","#C99800","#e58700","#e58700"))+
...
ggsave("k_pca_sva_root.pdf")
ggsave("k_pca_sva_root.jpg")
